...
setwd("C:/Bas/AquaBiota/Projekt/MEAD/Aurora/Februari_2021/Sec")
...
df=as.data.frame(read_excel("101816.client_OTUtable2.xlsx", skip = 2,sheet = 2))
...
foreach(aBool = sID, aPos = seq(1,length(sID),1))%do%{
  if(aBool == TRUE){
    gns=is.na(df$Genus[aPos])
    fmy=is.na(df$Genus[aPos])
    if(gns == FALSE){
      df$Species[aPos] = df$Genus[aPos]
      print(paste0("Added genus ",df$Genus[aPos]))}
    else if(fmy == FALSE){
      df$Species[aPos] = df$Family[aPos]
      print(paste0("Added family ",df$Family[aPos]))}
    else {df$Species[aPos] = df$Order[aPos]
    print(paste0("Added order ",df$Order[aPos]))
    }
}
}
...
write.csv(dfSpc$x, paste0(prjID,Primer[1], "_TillArtfakta.csv"), row.names = FALSE)
...
dfTrns=read.csv("MatchResult 2021-12-06 12.27.csv", sep = ";", fileEncoding = "UTF-8-BOM")
...
#Same script but looks in "Sökterm" for double lines instead of Vetenskapligt.namn
# myVal = 0 
...
for(x in seq(1, dim(dfTrns)[1], 1)){
  if(nchar(dfTrns$Svenskt.namn[x])<1){
    print(paste0("Adding latin taxa to SvensktNamn column: ", dfTrns$Vetenskapligt.namn[x]))
...
#meltDf$SampleID=gsub("^[0-9]{3,4}","", meltDf$SampleID)
