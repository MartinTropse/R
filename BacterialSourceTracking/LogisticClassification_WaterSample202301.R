...
logregfit0<-logreg(soutcome, bin=sbin, type=3, select=1, ntrees=3, nleaves=15, anneal.control=Annealing, tree.control=TreeControl)
logregfit1<-logreg(soutcome, bin=sbin, type=3, select=2, ntrees=c(1,3), nleaves=c(1,15), oldfit = logregfit0,anneal.control=Annealing, tree.control=TreeControl)
logregfit2<-logreg(select = 3, oldfit = logregfit1)

plot(logregfit2) # Plot with test score from k-fold validations of all models 

#Additional overview
#tapply(logregfit2$cvscores$test,list(logregfit2$cvscores$ntree,logregfit2$cvscores$nleaf),mean) #Mean of CV-score values per leave and tree
#tapply(logregfit2$cvscores$test,list(logregfit2$cvscores$ntree,logregfit2$cvscores$nleaf),sd) #SD of CV-score values per leave and tree
#logregfit1$alltrees
#summary(logregfit2)

#Choose the model based on the cv-test evaluation plot above 
logregfit1$allscores  #Use this for the model selection in the line below
cvBest_model<-logregfit1$alltrees[[41]] #Lowest cv-test score k-fold validation test.  
cvBestPredict=predict(logregfit1,newbin=tbin)

#Prediction / validation steps
bestModel=cvBestPredict$tr3.lf14 #Change to the used model
...
#Change these! To the tree, leaf and host used in the model to be exported
nTree = "T3"
nLeaf = "L14"
hostOrg = "ReinDeer"

#Name for the results summary file  
resSumry=paste0("ResultSumry_",hostOrg,nTree,nLeaf,"_",lubridate::as_date(Sys.time()),".txt")
...
perm.anova(Prediction ~ HostCatg, data=bestModel, nperm = 999,
           progress = TRUE)
sink()
